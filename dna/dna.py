...
with open(argv[1], 'r') as csv_file:
    ...
    # Separating the first line as a parameter and the rest as data
    for row in csv_reader:
        counter += 1
        if counter == 1:
            params = row
    ...
    # passing all elements except the first to the getTotalSTRs function
    total_STR = list(map(getTotalSTRs, params[1:]))

    # Cycling through data array and finding matching name
    for array in data:
